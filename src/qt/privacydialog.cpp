                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zCRN ought to be enough for anybody." - Bill Gates, 2017
    ui->zCRNpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzCRNSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzCRN->setEnabled(false);
        ui->pushButtonMintzCRN->setToolTip(tr("zCRN is currently disabled due to maintenance."));

        ui->pushButtonSpendzCRN->setEnabled(false);
        ui->pushButtonSpendzCRN->setToolTip(tr("zCRN is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zCRNpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzCRN_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zCRN is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Mint_zCRN, true));
        if (!ctx.isValid()) {
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zCRN...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zCRN in ") +
...
void PrivacyDialog::on_pushButtonSpendzCRN_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zCRN is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Send_zCRN, true));
        if (!ctx.isValid()) {
            // Unlock wallet was cancelled
            return;
        }
        // Wallet is unlocked now, sedn zCRN
        sendzCRN();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zCRN
    sendzCRN();
}

void PrivacyDialog::on_pushButtonZCrnControl_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;

    ZCrnControlDialog* zCrnControl = new ZCrnControlDialog(this);
    zCrnControl->setModel(walletModel);
    zCrnControl->exec();
}

void PrivacyDialog::setZCrnControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzCrnSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzCRN()
{
    ...
    double dAmount = ui->zCRNpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zCRN is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " CRN </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zCRN</b>";
    ...
    // use mints from zCrn selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZCrnControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZCrnControlDialog::GetSelectedMints();
    }
    ...
    if (walletModel && walletModel->getAddressTableModel()) {
        // If zCrn was spent successfully update the addressbook with the label
    ...
    // Clear zcrn selector in case it was used
    ZCrnControlDialog::listSelectedMints.clear();
    ui->labelzCrnSelected_int->setText(QString("0"));
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zCrn Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Crn, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zCrn Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zCRN </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zCRN "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zCRN "));
    ui->labelzCRNAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));

    // Display AutoMint status
    updateAutomintStatus();

    // Display global supply
    ui->labelZsupplyAmount->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zCRN </b> "));
    for (auto denom : libzerocoin::zerocoinDenomList) {
        ...
        QString strSupply = QString::number(nSupply) + " x " + QString::number(denom) + " = <b>" +
                            QString::number(nSupply*denom) + " zCRN </b> ";
        switch (denom) {
            case libzerocoin::CoinDenomination::ZQ_ONE:
...
{
    ui->labelzCRNSyncStatus->setVisible(fShow);
}
